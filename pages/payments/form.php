...
<select <?= $is_pelunasan ? 'disabled' : 'name="advertisement_id"' ?> id="advertisement_id" class="w-full px-3 py-2 border rounded-lg bg-white <?= $is_pelunasan ? 'bg-gray-100 text-gray-500 cursor-not-allowed' : '' ?>">
    ...
    // === GABUNGAN LOGIKA UPDATE DATA MAKSIMUM (NOMINAL & TANGGAL) ===
    function updateMaxAmount() {
        // Logika nominal jika mode pelunasan
        ...
        // 2. Set Batasan Tanggal Kontrak (Mengunci kalender HTML)
        if (endDate) {
            paymentDateInput.setAttribute('max', endDate);
            
            // Format tampilan tanggal ke Indonesia (DD-MM-YYYY) supaya user paham
    ...
    // === VALIDATION SAAT FORM DI-SUBMIT ===
    document.getElementById('paymentForm').addEventListener('submit', function(e) {
        ...
        // === PROTEKSI VALIDASI TANGGAL BARU ===
        ...
        if (maxDateString && paymentDateInput.value) {
            ...
            // Jika tanggal yang dipilih melampaui tanggal masa akhir kontrak iklan
        ...
        if(!isValid) {
            ...
            amountInput.value = cleanAmount; // bersihkan titik sebelum kirim ke PHP
        }
    });
</script>
</body>

...
if (isset($_GET['id'])) {
    ...
    $res = mysqli_query($conn, "SELECT * FROM clients WHERE id = '$edit_id'");
...
<head>
    ...
    <title><?= $isEdit ? 'Edit Klien' : 'Tambah Klien' ?></title>
...
<body class="bg-slate-50 text-slate-800 flex">
    ...
    <div class="flex-1 p-8 max-w-2xl">
        <h1 class="text-2xl font-bold text-slate-900 mb-6"><?= $isEdit ? 'Edit Klien' : 'Tambah Klien Baru' ?></h1>
        <div class="bg-white border border-slate-100 p-6 rounded-2xl shadow-sm">
            <form id="clientForm" method="POST" action="../../logic/client_process.php">
                ...
                <div class="mb-5">
                    ...
                    <input type="text" name="name" value="<?= $isEdit ? htmlspecialchars($editData['name']) : '' ?>" class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:outline-none focus:border-emerald-500" required>
                </div>
                <div class="mb-5">
                    <label class="block text-xs font-semibold text-slate-500 uppercase mb-2">No. Telepon</label>
                    <input type="text" name="phone" value="<?= $isEdit ? htmlspecialchars($editData['phone']) : '' ?>" class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:outline-none focus:border-emerald-500">
                </div>
                <div class="mb-6">
                    <label class="block text-xs font-semibold text-slate-500 uppercase mb-2">Alamat Kantor</label>
                    <textarea name="address" rows="4" class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:outline-none focus:border-emerald-500"><?= $isEdit ? htmlspecialchars($editData['address']) : '' ?></textarea>
                </div>
